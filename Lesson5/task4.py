...
# Первая часть - записываем
file = open("task4.txt", "w")
user_nums = input("Ввведите числа через пробел: ")
file.write(user_nums)
file.close()

# Втора часть - считаем
file = open("task4.txt", "r")
user_nums = file.readline()
result_list_sum = 0
list_sum, flag = find_sum(prepare_list(user_nums))
